	...
	export default {
		...
		methods: {
			update(){
				let that=this
				uni.request({
				url: "http://8.210.113.176:8080/users/alert/"+uni.getStorageSync('userId'),
				method: 'get',
				}).then(res=>{
			...
			switchA(e){
				...
				uni.request({
				url: "http://8.210.113.176:8080/users/alert/change/"+uni.getStorageSync('userId'),
				method: 'get',
				}).then(res=>{
					console.log('res',res)
				})
					...
					uni.showModal({
					    ...
					    success: function (res) {
					        if (res.confirm) {
					            ...
					            uni.request({
					            url: 'http://8.210.113.176:8080/item/update/all/'+uni.getStorageSync('userId')+'/'+that.day,
					            method: 'get',
					            }).then(res=>{
					            	console.log('res',res)
					            })
			...
			change(){
				...
				if(this.ivpwd==this.inpwd){
					uni.request({
						url:'http://8.210.113.176:8080/users/reset/pwd',
						method:'POST',
						data:{
